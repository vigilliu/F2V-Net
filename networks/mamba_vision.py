...
class MambaVision_sim(nn.Module):
    ...
    def forward_features_up(self, x, skip_list):
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                #print(f"(3)1Output min: {x.min()}, max: {x.max()}")
                x = layer_up(x)
            else:
                #print(f"(3)2Output min: {x.min()}, max: {x.max()}")
    ...
    def forward(self, x):
        #print(f"(mambain1)Input min: {x.min()}, max: {x.max()}")
        x, skip_list = self.forward_features(x)
        #print(f"(mambain2)Output min: {x.min()}, max: {x.max()}")
        x = self.forward_features_up(x, skip_list)
        #print(f"(mambain3)Output min: {x.min()}, max: {x.max()}")
        x = self.forward_final(x)
        #print(f"(mambain4)Output min: {x.min()}, max: {x.max()}")
        
        if self.num_classes == 1:
            x = torch.sigmoid(x)
            #print(f"(mambain5)Output min: {x.min()}, max: {x.max()}")
            return x
