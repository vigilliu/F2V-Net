...
# class CrossFrameMambaWithPE(nn.Module):
#     def __init__(self, num_frames=5, height=256, width=256, embed_dim=16, out_channels=1):
#         super().__init__()
...
#         # Step 1: 将每个像素点的时间序列 flatten 成一个序列 (B*H*W, T, 1)
#         x = x.permute(0, 2, 3, 1)  # (B, H, W, T)
#         x = x.reshape(B * H * W, T, 1)  # 每个像素位置是一个长度为 T 的时间序列

#         # Step 2: 映射到 embedding 空间
#         x = self.encoder(x)  # (B*H*W, T, embed_dim)

#         # Step 3: 加入时间位置编码
#         x = x + self.pos_embed  # (1, T, embed_dim) 会自动 broadcast

#         # Step 4: 用 Mamba 编码
#         x = self.mamba(x)  # (B*H*W, T, embed_dim)

#         # Step 5: 提取最后一帧的输出作为融合结果
#         x = self.decoder(x[:, -1, :])  # (B*H*W, out_channels)

#         # Step 6: reshape 回原始图像结构
...
class UNet_clips(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        # self.cross_frame = CrossFrameMambaWithPE(num_frames=5, height=256, width=256, embed_dim=16)
    ...
    def forward(self, x):
        #x = self.cross_frame(x)  # (B, 1, H, W)mamba
