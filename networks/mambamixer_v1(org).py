...
class MambaVision(nn.Module):
    ...
    def __init__(self,
        ...
        self.decode_head = SegFormerHead(num_classes, self.headin_channels, self.embedding_dim)
    ...
    def forward_features(self, x):
        print("Shape befotr patch embedding:", x.shape)  # 打印中间的形状
        x = self.patch_embed(x)
        print("Shape after patch embedding:", x.shape)  # 打印中间的形状
        # 保存不同尺度的输出
        outs = []
        outs.append(x)
